...
| Nodes cannot see or ping each other | Primary channel configuration mismatch (PSK/encryption key or channel name) | Align Index 0 channel settings (name and key) identically across both nodes. |
| Node not visible in BLE scans | Region not configured or Bluetooth disabled | Set the region first via serial CLI to unlock BLE advertising; verify `bluetooth.enabled` is `true`. |

---

## 🔰 Beginner Tips & Best Practices

1. **Antenna Safety:** Always connect a tuned antenna before powering on the board. Running the SX1262 transceiver without a load will permanently damage the power amplifier.
2. **Region Locks:** New or newly-flashed nodes disable BLE advertising and RF transmission until the region (e.g., `US`) is set.
3. **Name to Frequency Hashing:** Meshtastic hashes the primary channel name to calculate the RF frequency offset. If nodes have different primary channel names, they will tune to different frequencies and cannot communicate.
4. **Channel Encryption Modes (PSK):**
   - **Default (`psk="AQ=="`):** Uses the standard out-of-the-box Meshtastic AES-128 encryption key. Recommended for joining public meshes.
   - **Unencrypted (`psk=""`):** Turns off encryption completely. Used for fully open channels.
   - **Private/Secret Key (AES-256):** A custom base64 key generated by the app or CLI. Both nodes must have the exact same key string (case-sensitive, including any trailing `=` padding) to decrypt transmissions.
5. **Airtime Conservation (Channel Utilization):** LoRa channels are low-bandwidth. Keep telemetry and position updates at reasonable intervals (e.g., smart positioning enabled, node info at 3 hours/`10800s`) to avoid saturating the local channel.
6. **Hop Limits (Default 3):** Keep the message hop limit set to the default value of `3`. Setting it to `7` generates massive packet floods/storms, causing high latency and dropped packets across the local mesh.
7. **Headless Node Automation:** On screenless nodes like the ESP32-C3 Mini, configure the **Canned Messages** module to trigger automated status reports (like pings or pre-saved text alerts) using simple button-press patterns.


# Custom Firmware Reference: dxlr30-c3mini
Refer: https://github.com/meshtastic/firmware/blob/develop/variants/esp32c3/diy/esp32c3_super_min
